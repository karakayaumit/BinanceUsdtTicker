...
/// <summary>
/// Helper methods to build the local RSS bridge web application
/// without using top-level statements. Consumers may call
/// <see cref="BuildApp"/> and run the returned <see cref="WebApplication"/>.
/// </summary>
public static class RssBridgeProgram
{
    /// <summary>Builds the RSS bridge web application.</summary>
    ...
    private static async Task WriteRss(Stream output, string channelTitle, string channelLink, JsonElement items, Func<JsonElement, FeedItem> map)
    {
    ...
    private static FeedItem MapBybit(JsonElement it)
    {
    ...
    private static FeedItem MapKucoin(JsonElement it)
    {
    ...
    private record FeedItem(string Id, string Title, string? Description, string? Url, DateTimeOffset Timestamp);
}
